...
function abort_installer () {
  ...
  yad --title="Installer" --text="Flashing Aborted" --center --on-top --no-buttons --timeout=3;
  exit 1;
}
  ...
  if [ -b $2 ]; then
    if [ -x "$(which bmaptool)" ]; then
      ...
      $(while : ; do
        ...
        sleep 1
      done | tee /dev/ttymxc1 | yad --progress --title="Flashing..." --text="Writing content of $1 to $2" --center --on-top --button=cancel --auto-close; kill -2 $$) &
      if [ -f "${1//.wic*/}.wic.bmap" ]; then
        echo "bmaptool with bmap file: ${1//.wic*/}.wic.bmap"
        bmaptool copy $1 $2 --psplash-pipe /tmp/npipe 2>/dev/null 1>/dev/null &
      else
        echo "bmaptool with --no-bmap"
        bmaptool copy $1 $2 --nobmap --psplash-pipe /tmp/npipe 2>/dev/null 1>/dev/null &
      fi
      flash_pid=$!
...
if [ -n $wic_dir -a -n $wic_file -a -f $wic_dir/$wic_file ]; then
  flash_file=$(yad --file --filename=$wic_dir/$wic_file --file-filter "Wic Image Files: *.wic *.wic.gz" --center --on-top; if [ ! $? -eq 0 ]; then kill -2 $$; fi)
elif [ -n $wic_dir -a -d $wic_dir ]; then
  flash_file=$(yad --file --filename=$wic_dir --file-filter "Wic Image Files: *.wic *.wic.gz" --center --on-top; if [ ! $? -eq 0 ]; then kill -2 $$; fi)
else
  yad --title="Error. No wic image file found, and no image partition found." --center --on-top --timeout=3
...
if [ $complete -eq 1 ]; then
  yad --title="Flashing..." --text="Programming completed... Please power off the device..." --center --on-top --button=ok
else
  yad --title="Flashing..." --text="Programming Failed... Please try again..." --center --on-top --button=ok
fi
