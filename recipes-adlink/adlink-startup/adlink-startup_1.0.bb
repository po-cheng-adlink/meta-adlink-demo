...
do_install() {
  if ${@bb.utils.contains("DISTRO_FEATURES", 'systemd', 'true', 'false', d)}; then
    ...
    if ${@bb.utils.contains("MACHINE", 'lec-imx8mp', 'true', 'false', d)}; then
      install -m 0755 ${WORKDIR}/adlink-startup-imx8mp ${D}${sbindir}/${INITSCRIPT_NAME}
    fi
    ...
    if ${@bb.utils.contains("MACHINE", 'lec-imx8mp', 'true', 'false', d)}; then
      install -m 0755 ${WORKDIR}/adlink-startup-imx8mp ${D}${sysconfdir}/init.d/${INITSCRIPT_NAME}
    fi
  fi
}
