...
python () {
    ...
    if not tgtimg:
        bb.warn("Remeber to specify INSTALLER_TARGET_IMAGE(default: core-image-minimal) image to be packaged in data partition...\n")
    tgtpart = d.getVar("WIC_PARTITION_TYPE", expand=True)
    if not tgtpart:
        bb.warn("Remember to specify WIC_PARTITION_TYPE of data partition for storing INSTALLER_TARGET_IMAGE image file...\n")
}
